...
export default function Details() {
  ...
  const getScores = () => {
    ...
    if (data.scores.home.total > data.scores.away.total) {
      awayGoals = "gray-300";
    } else if (data.scores.home.total < data.scores.away.total) {
      homeGoals = "gray-300";
    }
    ...
    return (
      <>
        <Text className={a}>{data.scores.home.total}</Text>
        <Text className="text-5xl font-semibold text-gray-600 mx-3">:</Text>
        <Text className={b}>{data.scores.away.total}</Text>
      </>
    );
  };
  const getWinner = () => {
        ...
        <View>
          <DataTable>
            <DataTable.Header>
              <DataTable.Title
                textStyle={{
                  color: Colors.palette.secondary[950],
                  fontWeight: "bold",
                  fontSize: 16,
                }}
              >
            ...
            {Object.keys(data.scores.home.innings).map((inning, index) => {
              return (
                <DataTable.Row key={index + 1}>
                  ...
                  >
                    {data.scores.home.innings[inning] === null
                      ? "-"
                      : data.scores.home.innings[inning]}
                  </DataTable.Cell>
                  <DataTable.Cell
                    numeric
                    textStyle={{ color: Colors.palette.secondary[950] }}
                  >
                    {data.scores.away.innings[inning] === null
                      ? "-"
                      : data.scores.away.innings[inning]}
                  </DataTable.Cell>
                </DataTable.Row>
              );
            })}
            <DataTable.Row>
              ...
              <DataTable.Cell
                ...
                textStyle={{ color: Colors.palette.secondary[950] }}
              >
                {data.scores.home.hits}
              </DataTable.Cell>
              <DataTable.Cell
                numeric
                textStyle={{ color: Colors.palette.secondary[950] }}
              >
                {data.scores.away.hits}
              </DataTable.Cell>
            </DataTable.Row>
            <DataTable.Row>
              <DataTable.Cell
              ...
              <DataTable.Cell
                ...
                textStyle={{ color: Colors.palette.secondary[950] }}
              >
                {data.scores.home.errors === null
                  ? "-"
                  : data.scores.home.errors}
              </DataTable.Cell>
              <DataTable.Cell
                numeric
                textStyle={{ color: Colors.palette.secondary[950] }}
              >
                {data.scores.away.errors === null
                  ? "-"
                  : data.scores.away.errors}
              </DataTable.Cell>
            </DataTable.Row>
          </DataTable>
        </View>
